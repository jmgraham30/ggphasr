---
title: "Two-Dimensional ODE Systems"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Two-Dimensional ODE Systems}
...
where the derivatives depend only on the current state $(x, y)$, not on
time $t$ directly.  Two-dimensional systems are the natural setting for
**phase plane analysis**: rather than plotting each state variable
...
Convention B for 2D systems takes `x` and `y` as separate arguments
...
- Interior: $\left(\dfrac{\gamma}{\delta},\, \dfrac{\alpha}{\beta}\right)$
...
With $\alpha = 1$, $\beta = 0.5$, $\delta = 0.5$, $\gamma = 1$, the
interior equilibrium is at $(2, 2)$.
...
The x-nullcline (red, solid) and y-nullcline (blue, dashed) intersect
at the equilibria.  Their visual intersection immediately reveals the
...
> test is inconclusive about nonlinear stability.  The orbits are
> in fact closed curves (a consequence of a conserved quantity), but
> this is not guaranteed by linearization alone.  Compare with the Van
> der Pol oscillator in Section 5, which has a *stable limit cycle*
> that attracts all nearby trajectories.
...
$S = \gamma/\beta$) are at the epidemic peak; afterward $I$ declines.
All trajectories end on the x-axis ($I = 0$) — the disease-free
states — confirming that the epidemic always burns out in a closed
population.

The built-in `ode_sir()` provides the same model with an additional
`scale` parameter for switching between proportions and absolute counts:
...
The Van der Pol oscillator is a paradigmatic nonlinear oscillator with
a **stable limit cycle** — a closed trajectory that attracts all nearby
solutions:
...
Both trajectories — one starting inside the limit cycle and one
outside — converge to the same closed orbit.  This is the defining
property of a stable limit cycle.
...
```{r vdp-mu, fig.width = 9, fig.height = 4}
...
where $\alpha_{12}$ is the per-capita effect of species 2 on species 1,
and vice versa.  The four possible qualitative outcomes depend on the
...
into two **basins of attraction**.  Its stable manifold is the
separatrix — trajectories on one side converge to species 1 winning,
those on the other side to species 2 winning.
...
The equilibria are at $(\theta, \omega) = (k\pi, 0)$ for integer $k$.
Even multiples of $\pi$ are stable (pendulum hanging down); odd
...
The composable layer approach — `gg_flow_field() + gg_nullclines() +
gg_trajectory()` — makes it straightforward to build up a phase portrait
incrementally, while `gg_phase_plane()` provides a quick all-in-one
view for initial exploration.
