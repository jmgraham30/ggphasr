...
central to understanding the qualitative behavior of a system — it tells
us where trajectories end up, how they approach or depart from rest
states, and how the system responds to perturbations.

This vignette covers:

1.  Finding equilibria numerically with
    [`find_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/find_equilibrium.md)
2.  The Jacobian matrix and linearization
3.  Classifying equilibria with
    [`classify_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/classify_equilibrium.md)
4.  The trace-determinant plane
5.  Drawing stable and unstable manifolds with
...
For simple systems, equilibria can often be found by hand. For nonlinear
systems, numerical root-finding is more reliable.
[`find_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/find_equilibrium.md)
uses Newton–Raphson iteration (via
[`rootSolve::multiroot()`](https://rdrr.io/pkg/rootSolve/man/multiroot.html))
to locate equilibria from one or more starting points.
...
[`find_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/find_equilibrium.md)
places starting points on a regular grid over `xlim` × `ylim`, runs
...
> starting point is far from any root.
> [`find_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/find_equilibrium.md)
> guards against this by verifying that the residual
> $`\|f(x^*)\|_\infty`$ is below a tolerance threshold before accepting
> a root. Results are sorted by x-coordinate and deduplicated using a
> Euclidean distance threshold (`dedup_tol = 1e-4` by default).
...
behavior of nearby solutions. Near an equilibrium, the nonlinear system
...
[`classify_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/classify_equilibrium.md)
computes the Jacobian numerically via forward finite differences and
classifies the equilibrium using the **trace-determinant** criterion
(see Section 4):
...
and
[`classify_equilibrium()`](https://jmgraham30.github.io/ggphasr/reference/classify_equilibrium.md)
via [`lapply()`](https://rdrr.io/r/base/lapply.html) produces a tidy
summary table for all equilibria at once:
...
This table immediately reveals the qualitative structure: the origin and
the coexistence point $`(1, 1)`$ are saddles, while $`(3, 0)`$ and
...
**simple pendulum’s stable equilibrium** at the origin moves through the
trace-determinant plane as the damping coefficient $`b`$ increases:
...
![](equilibrium-analysis_files/figure-html/td-plane-1.png)
...
- **Stable manifold** $`W^s`$: the set of initial conditions that
  converge to the saddle as $`t \to +\infty`$. Locally aligned with the
  eigenvector of the negative eigenvalue.
- **Unstable manifold** $`W^u`$: the set of initial conditions that
  converge to the saddle as $`t \to -\infty`$ (equivalently, depart from
  it as $`t \to +\infty`$). Locally aligned with the eigenvector of the
  positive eigenvalue.

[`gg_manifolds()`](https://jmgraham30.github.io/ggphasr/reference/gg_manifolds.md)
computes both manifolds by:
...
![](equilibrium-analysis_files/figure-html/manifolds-linear-1.png)
...
![](equilibrium-analysis_files/figure-html/manifolds-competition-1.png)

Trajectories starting on opposite sides of the stable manifold (blue
dashed) converge to different outcomes — a concrete illustration of why
the separatrix is so important for understanding long-run behavior.
...
![](equilibrium-analysis_files/figure-html/toggle-phase-1.png)
...
#> 3 2.618034 0.381966    Stable node -2  0.5555555
```
...
![](equilibrium-analysis_files/figure-html/toggle-manifolds-1.png)
...
this boundary causes an irreversible switch from one stable state to the
other — this is precisely the “memory” property of the toggle switch.

------------------------------------------------------------------------

## 7 Bifurcation preview: how equilibria change with parameters

A **bifurcation** occurs when a qualitative change in the phase portrait
happens as a parameter crosses a critical value. `ggphasr` can
illustrate bifurcations by repeating the classification workflow across
...
#> 5 3.0   Unstable node 3.0   1    2.618034   0.0000000
```

At $`\mu = 0`$ the origin is a center ($`\tau = 0`$). For $`\mu > 0`$
the trace becomes positive and the origin is an unstable spiral — a Hopf
...
![](equilibrium-analysis_files/figure-html/sn-portraits-1.png)

The three panels show the phase line before the bifurcation (two
equilibria), at the bifurcation (one semi-stable equilibrium), and after
the bifurcation (no equilibria — all solutions escape to $`+\infty`$).
